...
class RandomDict(MutableMapping):
    ...
    def random_keys(self, k):
        if len(self) < k: raise KeyError("Not enough values in RandomDict")
        return [self.values[i][0] for i in random.sample(range(0, self.last_index), k)]
