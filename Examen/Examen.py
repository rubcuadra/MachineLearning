...
def funcionCosto(thetas, X, Y,_lambda):
    ...
    #Regularizacion:
    extra = _lambda*sum((thetas[i]**2 for i in range(1,len(thetas))))/(2*m)
    return J+extra

#Regresa un vector de thetas calculado usando gradiente desc
def aprende(theta,X,Y,_lambda,iteraciones=1500):
    ...
    for it in range(0,iteraciones): 
        ...
        for j in range(0, len(tempThetas) ):
            ...
            if j == 0:
                ...
                s = sum( ((sigmoidal(hipothesis(_x,theta))-_y)*_x[j]-theta[j]*_lambda/m)  for (_x,_y) in zip(fixedX,Y) )
            tempThetas[j] = theta[j] - (s/m)#En caso de haber alfa aqui era el *
        theta = tempThetas
    return theta
