...
def gradienteDescendenteMultivariable(X,Y,theta=None,alpha=0.1,iteraciones=100):
    ...
    #hipothesis(fixedX[0],theta)
    for it in range(0,iteraciones): 
        tempTheta = theta.copy() #Thetas temporales
        for j in range(0, len(theta) ):
            #FORMULA thetaI = (alfa/m)*sum( (hip(xi)-yi)*x[j]i  )
            tempTheta[j] = theta[j]-(alpha/len(X))*sum((hipothesis(valX,theta)-valY)*valX[j] for valX,valY in zip(fixedX,Y))
        theta = tempTheta   #Preparar para proxima iteracion
        jHistoria[it]  = calculaCosto(fixedX,Y,theta)   #Guardar costo
...
if __name__ == "__main__":
    ...
    jHistoria, thetas  = gradienteDescendenteMultivariable(nX,yData) 
    thetas2 = ecuacionNormal(xData,yData)
    print predicePrecio(xData[0],thetas)
    graficaError(jHistoria)
